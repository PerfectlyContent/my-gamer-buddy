...
CREATE TABLE IF NOT EXISTS cheat_codes (
    ...
    description TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT cheat_codes_game_title_platform_unique UNIQUE (game_id, title, platform)
);
...
CREATE TABLE IF NOT EXISTS quests (
    ...
    sort_order INTEGER DEFAULT 0,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT quests_game_name_unique UNIQUE (game_id, name)
);
...
FROM games g WHERE g.slug = 'gta'
ON CONFLICT ON CONSTRAINT cheat_codes_game_title_platform_unique DO NOTHING;
INSERT INTO cheat_codes (game_id, title, code, platform, category, description)
...
FROM games g WHERE g.slug = 'gta'
ON CONFLICT ON CONSTRAINT quests_game_name_unique DO NOTHING;
INSERT INTO quests (game_id, name, description, category, region, difficulty, is_missable, sort_order)
